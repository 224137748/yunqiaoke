    ...
    <div class="effect-pox">
      ...
      <div class="effect-content" v-if="!hide">
        <div class="effect-item" v-for="(item, i) in list" :key="i">
          <div class="effect-img"><img :src="item.user_info.avatar" alt=""></div>
          <div class="effect-center">
            <div class="effect-name">{{item.content_name}}</div>
            <div class="effect-centent">{{item.user_info.nickname}} · 下单奖励</div>
            <div class="effect-deTime">{{item.created_at}}</div>
          </div>
          <div class="effect-time">{{item.gem}}枚经验宝</div>
        </div>
      </div>
      <div v-else>暂无数据</div>
    </div>
...
import { getOrderTimes, getOrderRegister } from "../../../api/home.js";
export default {
  components: {
    tablist
    // headNav
  ...
  methods: {
    ...
    async getSay() {
      ...
      var res = await getOrderRegister(params);
      try {
        if (res.error_code === 0) {
          var data = res.data;
    ...
    async getData(statDay) {
      ...
      var res = await getOrderTimes(params);
      try {
        if (res.error_code === 0) {
