<template>
  ...
  <div class="server">
    <div class="bg" :style="'background-image: url('+body.thumb+')'">
      <i class="back"></i>
      <head-nav :title="'转让详情'" :navShow="false" :titleColor="'#FFFFFF'">
    ...
    <div class="scroller">
      <div>
        ...
        <div class="des" :class="!moreShow?'hide':''" v-html="body.content"></div>
  ...
  methods: {
    ...
    goPlaint() {
      this.$bridge.call("router", {
        url: "http://yunqiaoke_h5.sinmore.vip/#/partner/complaint?type=6&id=" + this.id,
        hideNav: 1,
    ...
    // 预约下单
    reser(id) {
      this.$router.push("/detail/reser?types=transfers&id=" + id);
    ...
    async guzhang() {
      ...
      that.$Indicator.open("加载中...");
      var params = { token: this.token, transfer_id: g.id };
    ...
    async getData() {
      ...
      var res = await getFTransferDetail(params);
      try {
        if (res.error_code === 0) {
          var data = res.data;
          // 计算当前时间
