<template>
  ...
  <div class="server">
    <div class="bg" :style="'background-image: url('+body.thumb+')'">
      <i class="back"></i>
      <head-nav :title="'融资详情'" :navShow="false" :titleColor="'#FFFFFF'">
    ...
    <div class="top">
      ...
      <div class="int">
        <div class="item">
          <div class="left">众筹金额</div>
          <div class="right">{{body.price}}</div>
        </div>
        <div class="item" v-if="body.category">
          <div class="left">众筹类型</div>
          <div class="right">{{body.category.name}}</div>
        </div>
        <!-- <div class="item">
          <div class="left">众筹轮次</div>
          <div class="right"></div>
        </div> -->
        <div class="item">
          <div class="left">众筹时间</div>
          <div class="right">{{body.start_at}} - {{body.ended_at}}</div>
        </div>
      </div>
    </div>
    <div class="deNav">
    ...
    <div class="scroller">
      <div>
        ...
        <!-- </div> -->
        <div class="des" :class="!moreShow?'hide':''" v-html="body.plan"></div>
      ...
      <div>
        <div class="conent mt">
          <div class="title"><i></i>融资回报</div>
        </div>
        <div class="des" :class="!moreShow2?'hide':''" v-html="body.back"></div>
      ...
      <div>
        <div class="conent mt">
          <div class="title"><i></i>风险措施</div>
        </div>
        <div class="des" :class="!moreShow3?'hide':''" v-html="body.measure"></div>
        <div class="more" @click="moreShow3 = true" v-if="!moreShow3">展开更多<img src="../../../assets/img/buddy/moreDown.png" alt=""></div>
        <div class="more" @click="moreShow3 = false" v-else>收回<img src="../../../assets/img/buddy/moreup.png" alt=""></div>
      </div>
      <div>
        <div class="conent mt">
  ...
  methods: {
    ...
    goPlaint() {
      this.$bridge.call("router", {
        url: "http://yunqiaoke_h5.sinmore.vip/#/partner/complaint?type=4&id=" + this.id,
        hideNav: 1,
    ...
    // 预约下单
    reser(id) {
      this.$router.push("/detail/reser?types=financings&id=" + id);
    ...
    async guzhang() {
      ...
      that.$Indicator.open("加载中...");
      var params = { token: this.token, financing_id: g.id };
    ...
    async getData() {
      ...
      var res = await getFinancingDetail(params);
      try {
        if (res.error_code === 0) {
          var data = res.data;
          // 计算当前时间
          ...
          data.ended_at = data.ended_at.replace(/-/g, "/");
          data.start_at = data.created_at.substring(0, 10).replace(/-/g, "/");
