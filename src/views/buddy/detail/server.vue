<template>
  ...
  <div class="server">
    <div class="bg" :style="'background-image: url('+body.thumb+')'">
      <i class="back"></i>
      <head-nav :title="'服务详情'" :navShow="false" :titleColor="'#FFFFFF'">
    ...
    <div class="scroller">
      ...
      <div>
        <div class="conent mt">
          <div class="title"><i></i>他的作品</div>
        </div>
        <div class="works">
          <div class="item" v-for="(item,i) in works" :key="i">
            <div class="bs"><img :src="item.thumb" alt=""></div>
...
export default {
  ...
  methods: {
    ...
    async getHeWork() {
      ...
      var res = await getHisWork(params);
      try {
        if (res.error_code === 0) {
          var data = res.data;
    ...
    goPlaint() {
      this.$bridge.call("router", {
        url: "http://yunqiaoke_h5.sinmore.vip/#/partner/complaint?type=1&id=" + this.id,
        hideNav: 1,
    ...
    // 预约下单
    reser(id) {
      this.$router.push("/detail/reser?types=services&id=" + id);
    ...
    async guzhang() {
      ...
      that.$Indicator.open("加载中...");
      var params = { token: this.token, service_id: g.id };
    ...
    async getData() {
      ...
      var res = await getServiceDetail(params);
      try {
        if (res.error_code === 0) {
          var data = res.data;
          // 计算当前时间
