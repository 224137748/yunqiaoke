<template>
  ...
  <div class="server">
    <div class="bg" :style="'background-image: url('+body.thumb+')'">
      <i class="back"></i>
      <head-nav :title="'派单详情'" :navShow="false" :titleColor="'#FFFFFF'">
    ...
    <div class="top">
      ...
      <div class="int">
        <div class="item" v-if="body.site">
          <div class="left">服务区域</div>
          <div class="right" :class="!fuwuHide?'fuwuhide':''" @click="fuwuHide=true">{{body.webCity}}</div>
        </div>
        ...
        <div class="item">
          ...
          <div class="right" v-if="body.verify === 1">审核</div>
          <div class="right" v-else>不审核</div>
        </div>
        <div class="item">
          <div class="left">完成次数</div>
          <div class="right">{{body.num}}</div>
        </div>
      </div>
    </div>
    <div class="deNav">
  ...
  methods: {
    ...
    goPlaint() {
      this.$bridge.call("router", {
        url: "http://yunqiaoke_h5.sinmore.vip/#/partner/complaint?type=5&id=" + this.id,
        hideNav: 1,
    ...
    // 预约下单
    reser(id) {
      this.$router.push("/detail/reser?types=dispatches&id=" + id);
    ...
    async guzhang() {
      ...
      that.$Indicator.open("加载中...");
      var params = { token: this.token, dispatch_id: g.id };
    ...
    async getData() {
      ...
      var res = await getDispatchDetail(params);
      try {
        if (res.error_code === 0) {
          var data = res.data;
          // 计算当前时间
