<template>
  <div class="transfer-form-page">
    <div class="main">
      ...
      <Topic type="transfer" @selectTopic="selectTopic" :tname="form.topicConfig.name"></Topic>
      <div class="form">
        ...
        <div class="input-textarea">
          ...
          <div class="textarea-wrap">
            <textarea placeholder="请输入" v-model="form.content"></textarea>
...
import { addTransfer, getTransfer } from "@/api/publish";
export default {
  name: "transfer-form-page",
  components: {
    actionBar,
    Topic
  },
  ...
  computed: {
    ...mapState({
      token: "token",
      saveForm: state => state.publish.transferForm
    })
  },
  mounted() {
    // 编辑
  ...
  methods: {
    ...
    // 保存草稿
    saveDraft(flag) {
      this.saveTransferForm(this.form);
      if (flag) {
        if (!this.form.thumb) {
          return this.$Toast({
            message: "请上传视频封面图片",
        ...
        if (this.form.content) {
          tempForm.content = this.form.content;
        }
        ...
        addTransfer(tempForm, true).then(res => {
          this.$Indicator.close();
          if (res.error_code === 0) {
            this.saveTransferForm(null);
            this.$Toast({
              message: "保存草稿成功",
              duration: 1500
            });
            // this.$router.push({ path: "/publish/plan", query: { type: 6 } });
    ...
    nextStep() {
      ...
      if (this.isAdd) {
        ...
        addTransfer(temForm, true).then(res => {
          this.$Indicator.close();
          if (res.error_code === 0) {
            ...
            this.$router.push({
              path: "/publish/plan",
              query: { type: 6, id: res.data.transfer_id }
            });
            // this.$router.push({ path: "/works", query: { type: 6 } });
          } else {
            this.$Toast(res.error_msg);
        ...
        addTransfer(temForm, false).then(res => {
          this.$Indicator.close();
          if (res.error_code === 0) {
            // this.$Toast("发布成功，系统审核中");
            this.saveTransferForm(null);
