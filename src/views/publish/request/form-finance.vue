<template>
  <div class="finance-form-page">
    <div class="main">
      ...
      <Topic type="finance" @selectTopic="selectTopic" :tname="form.topicConfig.name"></Topic>
      <div class="form">
        ...
        <div class="input-list flex flex-align-center">
          ...
          <span class="desc" @click="openPicker">{{
            form.ended_at ? form.ended_at : "请选择"
          }}</span>
          <span class="affix-img" @click="openPicker"></span>
        </div>
        <div class="input-textarea">
          <p class="title">计划</p>
          <div class="textarea-wrap">
            <textarea placeholder="请输入" v-model="form.plan"></textarea>
          </div>
        </div>
        <div class="input-textarea">
          <p class="title">回报</p>
          <div class="textarea-wrap">
            <textarea placeholder="请输入" v-model="form.back"></textarea>
          </div>
        </div>
        <div class="input-textarea">
          <p class="title">风险措施</p>
          <div class="textarea-wrap">
            <textarea placeholder="请输入" v-model="form.measure"></textarea>
          </div>
        </div>
      </div>
    ...
    <mt-datetime-picker ref="picker" type="date" v-model="form.date" year-format="{value} 年" month-format="{value} 月" date-format="{value} 日" @confirm="handleConfirmDate">
    </mt-datetime-picker>
  </div>
</template>
<script>
...
import { addFinance, getFinance } from "@/api/publish";
export default {
  name: "finance-form-page",
  components: {
    actionBar,
    Topic
  },
  ...
  computed: {
    ...mapState({
      token: "token",
      saveForm: state => state.publish.financeForm
    })
  },
  mounted() {
    // 编辑
    if (this.$route.query.id) {
      ...
      getFinance({
        token: this.token,
        financing_id: this.$route.query.id
      }).then(res => {
        ...
        if (res.error_code === 0) {
          ...
          this.form = {
            ...
            lng: "",
            lat: "", // 经纬度
            ended_at: modifyForm.ended_at || "",
            plan: modifyForm.plan || "",
            back: modifyForm.back || "",
            measure: modifyForm.measure || "",
  ...
  methods: {
    ...
    selectTopic() {
      ...
      this.saveDraft();
      this.$router.push({ path: "/publish/topic", query: { type: "finance" } });
    },
    // 保存草稿
    saveDraft(flag) {
      this.saveFinanceForm(this.form);
      if (flag) {
        if (!this.form.thumb) {
          return this.$Toast({
            message: "请上传视频封面图片",
        ...
        if (this.form.ended_at) {
          tempForm.ended_at = this.form.ended_at;
        }
        if (this.form.plan) {
          tempForm.plan = this.form.plan;
        }
        if (this.form.back) {
          tempForm.back = this.form.back;
        }
        if (this.form.measure) {
          tempForm.measure = this.form.measure;
        }
        ...
        addFinance(tempForm, true).then(res => {
          this.$Indicator.close();
          if (res.error_code === 0) {
            this.$Toast({
              message: "保存草稿成功",
            ...
            this.saveFinanceForm(null);
            // this.$router.push({ path: "/publish/plan", query: { type: 4 } });
    ...
    nextStep() {
      ...
      if (this.isAdd) {
        ...
        addFinance(temForm, true).then(res => {
          this.$Indicator.close();
          if (res.error_code === 0) {
            ...
            this.$router.push({
              path: "/publish/plan",
              query: { type: 4, id: res.data.financing_id }
            });
            // this.$router.push({ path: "/works", query: { type: 4 } });
          } else {
            this.$Toast(res.error_msg);
        ...
        addFinance(temForm, false).then(res => {
          this.$Indicator.close();
          if (res.error_code === 0) {
            // this.$Toast("发布成功，系统审核中");
            this.saveFinanceForm(null);
