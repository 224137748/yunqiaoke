<template>
  <div class="invest-form-page">
    <div class="main">
      ...
      <Topic type="invest" @selectTopic="selectTopic" :tname="form.topicConfig.name"></Topic>
      <div class="form">
        <div class="input-list flex flex-align-center">
          <span class="title">投资标题</span>
        ...
        <div class="input-textarea">
          <p class="title">要求</p>
          <div class="textarea-wrap">
            <textarea placeholder="请输入" v-model="form.claim"></textarea>
          </div>
        </div>
      </div>
...
import { addInvest, getInvest } from "@/api/publish";
export default {
  name: "invest-form-page",
  components: {
    actionBar,
    Topic
  },
  ...
  computed: {
    ...mapState({
      token: "token",
      saveForm: state => state.publish.investForm
    })
  },
  mounted() {
    // 编辑
  ...
  methods: {
    ...
    selectTopic() {
      ...
      this.saveDraft();
      this.$router.push({ path: "/publish/topic", query: { type: "invest" } });
    },
    // 保存草稿
    saveDraft(flag) {
      this.saveInvestForm(this.form);
      if (flag) {
        if (!this.form.thumb) {
          return this.$Toast({
            message: "请上传视频封面图片",
        ...
        if (this.form.claim) {
          tempForm.claim = this.form.claim;
        }
        ...
        addInvest(tempForm, true).then(res => {
          this.$Indicator.close();
          if (res.error_code === 0) {
            this.$Toast({
              message: "保存草稿成功",
            ...
            this.saveInvestForm(null);
            // this.$router.push({ path: "/publish/plan", query: { type: 3 } });
    ...
    nextStep() {
      ...
      if (this.isAdd) {
        ...
        addInvest(temForm, true).then(res => {
          this.$Indicator.close();
          if (res.error_code === 0) {
            ...
            this.$router.push({
              path: "/publish/plan",
              query: { type: 3, id: res.data.investment_id }
            });
            // this.$router.push({ path: "/works", query: { type: 3 } });
          } else {
            this.$Toast(res.error_msg);
        ...
        addInvest(temForm, false).then(res => {
          this.$Indicator.close();
          if (res.error_code === 0) {
            // this.$Toast("发布成功，系统审核中");
            this.saveInvestForm(null);
