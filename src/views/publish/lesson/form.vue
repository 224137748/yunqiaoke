<template>
  <div class="lesson-form-page">
    <div class="main">
      ...
      <Topic type="lesson" @selectTopic="selectTopic" :tname="form.topicConfig.name"></Topic>
      <div class="form">
...
import { addLesson, getLesson } from "@/api/publish";
export default {
  name: "lesson-form-page",
  components: {
    actionBar,
    Topic
  },
  ...
  computed: {
    ...mapState({
      token: "token",
      saveForm: state => state.publish.lessonForm
    })
  },
  mounted() {
    // 编辑
    if (this.$route.query.id) {
      ...
      getLesson({ token: this.token, course_id: this.$route.query.id }).then(
        res => {
  ...
  methods: {
    ...
    selectTopic() {
      ...
      this.saveDraft();
      this.$router.push({ path: "/publish/topic", query: { type: "lesson" } });
    },
    // 保存草稿
    saveDraft(flag) {
      this.saveLessonForm(this.form);
      if (flag) {
        if (!this.form.thumb) {
          return this.$Toast({
            message: "请上传视频封面图片",
        ...
        addLesson(tempForm, true).then(res => {
          this.$Indicator.close();
          if (res.error_code === 0) {
            this.$Toast({
              message: "保存草稿成功",
            ...
            this.saveLessonForm(null);
            // this.$router.push({ path: "/publish/plan", query: { type: 7 } });
    ...
    nextStep() {
      ...
      if (this.isAdd) {
        ...
        addLesson(temForm, true).then(res => {
          this.$Indicator.close();
          if (res.error_code === 0) {
            ...
            this.$router.push({
              path: "/publish/plan",
              query: { type: 7, id: res.data.course_id }
            });
            // this.$router.push({ path: "/works", query: { type: 7 } });
          } else {
            this.$Toast(res.error_msg);
        ...
        addLesson(temForm, false).then(res => {
          this.$Indicator.close();
          if (res.error_code === 0) {
            // this.$Toast("发布成功，系统审核中");
            this.saveLessonForm(null);
