...
import { addSkill, getSkill } from "@/api/publish";
export default {
  name: "skill-form-page",
  components: {
    actionBar,
    Topic
  },
  ...
  computed: {
    ...mapState({
      token: "token",
      saveForm: state => state.publish.skillForm
    })
  },
  mounted() {
    // 编辑
    if (this.$route.query.id) {
      ...
      getSkill({ token: this.token, skill_id: this.$route.query.id }).then(
        res => {
  ...
  methods: {
    ...
    reSelectArea() {
      this.saveDraft();
      this.$router.push({ path: "/publish/area", query: { type: "skill" } });
    },
    /**
     * 重选服务行业
     */
    ...
    selectTopic() {
      ...
      this.saveDraft();
      this.$router.push({ path: "/publish/topic", query: { type: "skill" } });
    },
    // 保存草稿
    saveDraft(flag) {
      this.saveSkillForm(this.form);
      if (flag) {
        if (!this.form.thumb) {
          return this.$Toast({
            message: "请上传视频封面图片",
        ...
        addSkill(tempForm, true).then(res => {
          this.$Indicator.close();
          if (res.error_code === 0) {
            this.$Toast({
              message: "保存草稿成功",
            ...
            this.saveSkillForm(null);
            // this.$router.push({ path: "/publish/plan", query: { type: 2 } });
    ...
    nextStep() {
      ...
      if (this.isAdd) {
        ...
        addSkill(temForm, true).then(res => {
          this.$Indicator.close();
          if (res.error_code === 0) {
            ...
            this.$router.push({
              path: "/publish/plan",
              query: { type: 2, id: res.data.skill_id }
            });
            // this.$router.push({ path: "/works", query: { type: 2 } });
          } else {
            this.$Toast(res.error_msg);
        ...
        addSkill(temForm, false).then(res => {
          this.$Indicator.close();
          if (res.error_code === 0) {
            // this.$Toast("发布成功，系统审核中");
            this.saveSkillForm(null);
