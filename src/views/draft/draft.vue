<template>
  <div class="draft-page" :data-type="showType">
    <div class="main">
      <div class="tab-bar">
        <div class="tab-item">
          <span :class="{ active: showType === 1 }" @click="selectShowType(1)"
            >服务</span
          >
        </div>
        <div class="tab-item">
          <span :class="{ active: showType === 5 }" @click="selectShowType(5)"
            >派单</span
          >
        </div>
        <div class="tab-item">
          <span :class="{ active: showType === 6 }" @click="selectShowType(6)"
            >转让</span
          >
        </div>
        <div class="tab-item">
          <span :class="{ active: showType === 3 }" @click="selectShowType(3)"
            >投资</span
          >
        </div>
        <div class="tab-item">
          <span :class="{ active: showType === 4 }" @click="selectShowType(4)"
            >融资</span
          >
        </div>
        <div class="tab-item">
          <span :class="{ active: showType === 7 }" @click="selectShowType(7)"
            >公开课</span
          >
        </div>
        <div class="tab-item">
          <span :class="{ active: showType === 2 }" @click="selectShowType(2)"
            >技能</span
          >
        </div>
      </div>
  ...
  methods: {
    ...
    async getData() {
      ...
      try {
        ...
        let res = await this.API.getContentList(params);
    ...
    async deletContentItem(item) {
      this.$MessageBox({
        ...
        if (val === "confirm") {
          try {
            this.$Indicator.open("加载中...");
            let res = await this.API.deletContent(this.token, [item.id]);
            if (res.error_code === 0) {
    ...
    async putOnlineContentItem(item) {
      try {
        this.$Indicator.open("加载中...");
        let res = await this.API.editContent(this.token, item.id);
        if (res.error_code === 0) {
          let index = this.contentList.findIndex(el => {
            return el.id === item.id;
          });
      ...
      .content-item {
        ...
        &.showType2 {
          /deep/ .mint-cell-swipe-button:first-child {
            &::before {
              background: url("../../assets/img/user/edit-conent.png") no-repeat
                left top/100% 100%;
            }
          }
