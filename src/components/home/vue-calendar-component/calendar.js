export default {
  ...
  getLeftArr(date) {
    ...
    for (let i = 0; i < leftNum; i++) {
      var myMonths = preDate.getMonth() + 1 < 10 ? "0" + (preDate.getMonth() + 1) : preDate.getMonth() + 1;
      const nowTime = preDate.getFullYear() + "/" + myMonths + "/" + (num + i);
      arr.push({
        id: num + i,
        date: nowTime,
        isToday: false,
  ...
  getRightArr(date) {
    ...
    for (let i = 0; i < _length; i++) {
      var myMonths = nextDate.getMonth() + 1 < 10 ? "0" + (nextDate.getMonth() + 1) : nextDate.getMonth() + 1;
      var myDays = (i + 1) < 10 ? "0" + (i + 1) : i + 1;
      const nowTime = nextDate.getFullYear() + "/" + myMonths + "/" + myDays;
      arr.push({
        id: i + 1,
        date: nowTime,
        isToday: false,
  ...
  getMonthListNoOther(date) {
    ...
    var myMonths = month < 10 ? "0" + month : month;

    const toDay = this.dateFormat(new Date());
    for (let i = 0; i < num; i++) {
      var myDays = (i + 1) < 10 ? "0" + (i + 1) : i + 1;

      const nowTime = year + "/" + myMonths + "/" + myDays;
      arr.push({
        id: i + 1,
        date: nowTime,
